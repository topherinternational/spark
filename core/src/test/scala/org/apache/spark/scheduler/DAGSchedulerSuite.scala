...
class DAGSchedulerSuite extends SparkFunSuite with LocalSparkContext with TimeLimits {
  ...
  test("All shuffle files on the slave should be cleaned up when slave lost") {
    ...
    complete(taskSets(2), Seq(
      (FetchFailed(makeShuffleLocationSeq("exec-hostA2", "hostA", 12345),
  ...
  test("run trivial shuffle") {
    ...
    assert(mapOutputTracker.getMapSizesByShuffleLocation(shuffleId, 0).map(_._1).toSet ===
      HashSet(makeShuffleLocationSeq("hostA"), makeShuffleLocationSeq("hostB")))
  ...
  test("run trivial shuffle with fetch failure") {
    ...
    complete(taskSets(1), Seq(
      (Success, 42),
      (FetchFailed(makeShuffleLocationSeq("hostA"), shuffleId, 0, 0, "ignored"), null)))
  ...
  private def completeNextStageWithFetchFailure(
    ...
    complete(stageAttempt, stageAttempt.tasks.zipWithIndex.map { case (task, idx) =>
      (FetchFailed(makeShuffleLocationSeq("hostA"),
  ...
  test("trivial shuffle with multiple fetch failures") {
    ...
    runEvent(makeCompletionEvent(
      taskSets(1).tasks(0),
      FetchFailed(makeShuffleLocationSeq("hostA"), shuffleId, 0, 0, "ignored"),
      null))
    sc.listenerBus.waitUntilEmpty(WAIT_TIMEOUT_MILLIS)
    ...
    runEvent(makeCompletionEvent(
      taskSets(1).tasks(0),
      FetchFailed(makeShuffleLocationSeq("hostA"), shuffleId, 1, 1, "ignored"),
      null))
  ...
  test("late fetch failures don't cause multiple concurrent attempts for the same map stage") {
    ...
    runEvent(makeCompletionEvent(
      taskSets(1).tasks(1),
      FetchFailed(makeShuffleLocationSeq("hostB"), shuffleId, 1, 1, "ignored"),
      null))
  ...
  test("ignore late map task completions") {
    ...
    assert(mapOutputTracker.getMapSizesByShuffleLocation(shuffleId, 0).map(_._1).toSet ===
      HashSet(makeShuffleLocationSeq("hostB"), makeShuffleLocationSeq("hostA")))
  ...
  test("run trivial shuffle with out-of-band executor failure and retry") {
    ...
    assert(mapOutputTracker.getMapSizesByShuffleLocation(shuffleId, 0).map(_._1).toSet ===
      HashSet(makeShuffleLocationSeq("hostC"), makeShuffleLocationSeq("hostB")))
  ...
  test("reduce tasks should be placed locally with map output") {
    ...
    complete(taskSets(0), Seq(
      (Success, makeMapStatus("hostA", 1))))
    assert(mapOutputTracker.getMapSizesByShuffleLocation(shuffleId, 0).map(_._1).toSet ===
      HashSet(makeShuffleLocationSeq("hostA")))
  ...
  test("map stage submission with multiple shared stages and failures") {
    ...
    complete(taskSets(1), Seq(
      ...
      (FetchFailed(makeShuffleLocationSeq("hostA"), dep1.shuffleId, 0, 0, "ignored"), null)))
    scheduler.resubmitFailedStages()
    ...
    assert(mapOutputTracker.getMapSizesByShuffleLocation(dep2.shuffleId, 0).map(_._1).toSet ===
      HashSet(makeShuffleLocationSeq("hostB"), makeShuffleLocationSeq("hostD")))
    ...
    complete(taskSets(4), Seq(
      (Success, 52),
      (FetchFailed(makeShuffleLocationSeq("hostD"), dep2.shuffleId, 0, 0, "ignored"), null)))
    scheduler.resubmitFailedStages()
  ...
  test("Trigger mapstage's job listener in submitMissingTasks") {
    ...
    complete(taskSets(2), Seq(
      (Success, makeMapStatus("hostC", rdd2.partitions.length))))
    assert(mapOutputTracker.getMapSizesByShuffleLocation(dep1.shuffleId, 0).map(_._1).toSet ===
        Set(makeShuffleLocationSeq("hostC"), makeShuffleLocationSeq("hostB")))
      ...
      rdd1.map {
        case (x, _) if (x == 1) =>
          throw new FetchFailedException(
            makeShuffleLocationSeq("1", "1", 1), shuffleHandle.shuffleId, 0, 0, "test")
        case (x, _) => x
      }.count()
    }
  ...
  test("[SPARK-19263] DAGScheduler should not submit multiple active tasksets," +
    ...
    runEvent(makeCompletionEvent(
      taskSets(1).tasks(0),
      FetchFailed(makeShuffleLocationSeq("hostA"), shuffleIdA, 0, 0,
  ...
  test("SPARK-23207: retry all the succeeding stages when the map stage is indeterminate") {
    ...
    runEvent(makeCompletionEvent(
      taskSets(2).tasks(0),
      FetchFailed(makeShuffleLocationSeq("hostC"), shuffleId2, 0, 0, "ignored"),
      null))
    ...
    runEvent(makeCompletionEvent(
      taskSets(3).tasks(0),
      FetchFailed(makeShuffleLocationSeq("hostA"), shuffleId1, 0, 0, "ignored"),
      null))
...
object DAGSchedulerSuite {
  ...
  def makeShuffleLocationSeq(execId: String, host: String, port: Int): Seq[ShuffleLocation] = {
    Seq(DefaultMapShuffleLocations.get(makeBlockManagerId(host)))
  }

  def makeShuffleLocationSeq(host: String): Seq[ShuffleLocation] = {
    Seq(DefaultMapShuffleLocations.get(makeBlockManagerId(host)))
  }
}
