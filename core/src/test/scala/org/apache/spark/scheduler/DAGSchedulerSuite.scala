...
class DAGSchedulerSuite extends SparkFunSuite with LocalSparkContext with TimeLimits {
  ...
  test("All shuffle files on the slave should be cleaned up when slave lost") {
    ...
    complete(taskSets(2), Seq(
      (FetchFailed(BlockManagerId(
        "exec-hostA2", "hostA", 12345), firstShuffleId, 0, 0L, 0, "ignored"), null)
    ))
  ...
  test("run trivial shuffle") {
    ...
    assert(mapOutputTracker
      .getPartitionMetadata(shuffleId, 0).blocksByExecutorId.map(_._1.get).toSet ===
  ...
  test("run trivial shuffle with fetch failure") {
    ...
    complete(taskSets(1), Seq(
      (Success, 42),
      (FetchFailed(makeBlockManagerId("hostA"), shuffleId, 0, 0L, 0, "ignored"), null)))
    ...
    assert(mapOutputTracker
      .getPartitionMetadata(shuffleId, 0).blocksByExecutorId.map(_._1.get.host).toSet ===
      HashSet("hostA", "hostB"))
    ...
    test(s"shuffle files $maybeLost when $eventDescription") {
      ...
      if (expectFileLoss) {
        intercept[MetadataFetchFailedException] {
          mapOutputTracker.getPartitionMetadata(shuffleId, 0)
        }
      } else {
        assert(mapOutputTracker
          .getPartitionMetadata(shuffleId, 0).blocksByExecutorId.map(_._1.get).toSet ===
  ...
  private def completeNextStageWithFetchFailure(
    ...
    complete(stageAttempt, stageAttempt.tasks.zipWithIndex.map { case (task, idx) =>
      (FetchFailed(makeBlockManagerId("hostA"), shuffleDep.shuffleId, 0, 0L, idx, "ignored"), null)
    }.toSeq)
  }
  ...
  test("trivial shuffle with multiple fetch failures") {
    ...
    runEvent(makeCompletionEvent(
      taskSets(1).tasks(0),
      FetchFailed(makeBlockManagerId("hostA"), shuffleId, 0, 0L, 0, "ignored"),
      null))
    sc.listenerBus.waitUntilEmpty(WAIT_TIMEOUT_MILLIS)
    ...
    runEvent(makeCompletionEvent(
      taskSets(1).tasks(0),
      FetchFailed(makeBlockManagerId("hostA"), shuffleId, 1, 0L, 1, "ignored"),
      null))
  ...
  test("late fetch failures don't cause multiple concurrent attempts for the same map stage") {
    ...
    assert(mapOutputTracker
      ...
      HashSet("hostA", "hostB"))
    assert(mapOutputTracker
      .getPartitionMetadata(shuffleId, 1).blocksByExecutorId.map(_._1.get.host).toSet ===
      HashSet("hostA", "hostB"))
    ...
    runEvent(makeCompletionEvent(
      taskSets(1).tasks(1),
      FetchFailed(makeBlockManagerId("hostB"), shuffleId, 1, 0L, 1, "ignored"),
      null))
  ...
  test("don't submit stage until its dependencies map outputs are registered (SPARK-5259)") {
    ...
    runEvent(makeCompletionEvent(
      taskSets(1).tasks(0),
      FetchFailed(null, firstShuffleId, 2, 0L, 0, "Fetch failed"),
      null))
    ...
    for {
      ...
      val statuses = mapOutputTracker.getPartitionMetadata(stage, reduceIdx)
  ...
  test("register map outputs correctly after ExecutorLost and task Resubmitted") {
    ...
    (0 until reduceRdd.partitions.length).foreach { reduceIdx =>
      val statuses = mapOutputTracker.getPartitionMetadata(0, reduceIdx)
      // really we should have already thrown an exception rather than fail either of these
      // asserts, but just to be extra defensive let's double check the statuses are OK
      assert(statuses != null)
      assert(statuses.blocksByExecutorId.nonEmpty)
  ...
  test("stage used by two jobs, some fetch failures, and the first job no longer active " +
    ...
    complete(taskSets(1), Seq(
      (FetchFailed(makeBlockManagerId("hostA"), shuffleDep1.shuffleId, 0, 0L, 0, "ignored"), null)))
    scheduler.resubmitFailedStages()
  ...
  test("recursive shuffle failures") {
    ...
    complete(taskSets(2), Seq(
      (FetchFailed(
        makeBlockManagerId("hostA"), shuffleDepTwo.shuffleId, 0, 0L, 0, "ignored"), null)))
    // TODO assert this:
    // blockManagerMaster.removeExecutor("exec-hostA")
  ...
  test("stage level active shuffle tracking with multiple dependents") {
    ...
    complete(stageAttempt, stageAttempt.tasks.zipWithIndex.map {
      case (task, idx) =>
        (Success, makeMapTaskResult("host" + ('A' + idx).toChar, 1))
    }.toSeq)
    assert(mapOutputTracker.shuffleStatuses(shuffleId).isActive === true)
  ...
  test("map stage submission with multiple shared stages and failures") {
    ...
    assert(mapOutputTracker
      .getPartitionMetadata(dep1.shuffleId, 0).blocksByExecutorId.map(_._1.get).toSet ===
      HashSet(makeBlockManagerId("hostA"), makeBlockManagerId("hostB")))
    ...
    complete(taskSets(1), Seq(
      ...
      (FetchFailed(makeBlockManagerId("hostA"), dep1.shuffleId, 0, 0L, 0, "ignored"), null)))
    scheduler.resubmitFailedStages()
    ...
    assert(mapOutputTracker
      .getPartitionMetadata(dep2.shuffleId, 0).blocksByExecutorId.map(_._1.get).toSet ===
    ...
    complete(taskSets(4), Seq(
      (Success, 52),
      (FetchFailed(makeBlockManagerId("hostD"), dep2.shuffleId, 0, 0L, 0, "ignored"), null)))
    scheduler.resubmitFailedStages()
      ...
      rdd1.map {
        case (x, _) if (x == 1) =>
          throw new FetchFailedException(
            BlockManagerId("1", "1", 1), shuffleHandle.shuffleId, 0, 0L, 0, "test")
        case (x, _) => x
      }.count()
    }
  ...
  test("[SPARK-19263] DAGScheduler should not submit multiple active tasksets," +
    ...
    runEvent(makeCompletionEvent(
      taskSets(1).tasks(0),
      FetchFailed(makeBlockManagerId("hostA"), shuffleIdA, 0, 0L, 0,
  ...
  test("SPARK-23207: retry all the succeeding stages when the map stage is indeterminate") {
    ...
    runEvent(makeCompletionEvent(
      taskSets(2).tasks(0),
      FetchFailed(makeBlockManagerId("hostC"), shuffleId2, 0, 0L, 0, "ignored"),
      null))
    ...
    runEvent(makeCompletionEvent(
      taskSets(3).tasks(0),
      FetchFailed(makeBlockManagerId("hostA"), shuffleId1, 0, 0L, 0, "ignored"),
      null))
...
object DAGSchedulerSuite {
  def makeMapTaskResult(host: String, reduces: Int, sizes: Byte = 2): MapTaskResult =
    MapTaskResult(
