...
private[spark] object MapOutputTracker extends Logging {
  ...
  def convertMapStatuses(
      ...
      startPartition: Int,
      endPartition: Int,
      statuses: Array[MapStatus]): Iterator[(Option[MapShuffleLocations], Seq[(BlockId, Long)])] = {
    assert (statuses != null)
    val splitsByAddress = new HashMap[Option[MapShuffleLocations], ListBuffer[(BlockId, Long)]]
        ...
        for (part <- startPartition until endPartition) {
          ...
          if (size != 0) {
            if (status.mapShuffleLocations == null) {
              ...
              val shuffleLoc = status.mapShuffleLocations
