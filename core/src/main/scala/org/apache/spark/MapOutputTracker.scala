...
private[spark] abstract class MapOutputTracker(conf: SparkConf) extends Logging {
  ...
  def getMapSizesByShuffleLocation(shuffleId: Int, reduceId: Int)
      : Iterator[(Option[ShuffleLocation], Seq[(BlockId, Long, BlockManagerId)])] = {
...
private[spark] object MapOutputTracker extends Logging {
  ...
  def convertMapStatuses(
    ...
    val splitsByAddress = new HashMap[Option[ShuffleLocation],
      ListBuffer[(BlockId, Long, BlockManagerId)]]
          ...
          if (size != 0) {
            if (status.mapShuffleLocations == null) {
              ...
              splitsByAddress.getOrElseUpdate(Option.empty, ListBuffer()) +=
                ((ShuffleBlockId(shuffleId, mapId, part), size, blockManagerId))
            } else {
